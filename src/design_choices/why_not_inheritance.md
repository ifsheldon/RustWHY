...
其实多态在**一定程度**上也可以说是为了代码复用，所以终极的原因还是代 cheng 码 xu 可 yuan 复 tou 用 lan。所以这也表现在继承的具体实现上了。通过继承，程序员可以不用写父类已经有的方法，也可以不用声明父类已经有的属性，这就让代码更加能被复用了。

那么看起来继承全是好处，所以程序员和继承 forever and forever a 100 years!? 
